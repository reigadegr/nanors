...
### 文件头部
每个crate的 `lib.rs`或`main.rs` 文件必须包含严格的 linting 配置：
```rust
#![deny(
    clippy::all,
    clippy::nursery,
...
项目使用 **IvorySQL** 作为数据库（基于 PostgreSQL 构建，与 PostgreSQL 完美兼容）。

### 连接信息
- 数据库类型：IvorySQL / PostgreSQL
- 数据库名：`nanors`
- 用户名：`reigadegr`
- 密码：`1234`
...
**nanors_entities** crate 包含由 `sea-orm-cli` 自动生成的数据库实体代码：
- **不要手动修改** `nanors_entities` 中的大部分代码
- 当数据库结构发生变化时（修改字段、删除表、新建表），必须重新生成实体：
  ```bash
  sea-orm-cli generate entity --with-serde=both -o nanors_entities/src
  ```

### 数据库迁移流程
1. 修改数据库 schema（字段、表结构等）
2. 执行 `sea-orm-cli generate entity` 命令重新生成实体
3. 运行 `./debug.sh` 确保代码编译通过
...
- **nanors_config**：配置管理
- **nanors_entities**：由 sea-orm-cli 自动生成的数据库实体（**不要手动修改**）
